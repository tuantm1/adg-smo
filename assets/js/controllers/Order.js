MetronicApp.controller('DeliveryOrderCtr',
        ...
        function ($scope, $state, $resource, $stateParams, NgTableParams, ApiQuery, $rootScope, $http, $timeout) {
            ...
            $scope.data = {
                ...
                "IT_KUNNR": [{
                    "SIGN": "I",
                    "OPTION": "EQ",
                    "CUSTOMER_VENDOR_LOW": "1000000633"
                }]
            };


            $scope.submitForm = function () {
                $scope.data.IT_BLDAT[0].DOC_DATE_LOW = $('#reportrange').data('daterangepicker').startDate.format('YYYYMMDD');
                $scope.data.IT_BLDAT[0].DOC_DATE_HIGH = $('#reportrange').data('daterangepicker').endDate.format('YYYYMMDD');
                $scope.data.IT_KUNNR[0].CUSTOMER_VENDOR_LOW = $rootScope.userData.kunnr;

                ApiQuery.post('/list_do', $scope.data).then(function (res) {
                    $scope.obs = res.data.ET_DOLIST;
                    $scope.obs.forEach(function (value) {
                        value.MATNR = value.MATNR.replace(/^0*/g, '');
                    })
                })
                $rootScope.screen = '2';
            };
    ...
    ['$scope', '$state', '$resource', '$stateParams', 'NgTableParams', 'ApiQuery', '$rootScope', '$http', '$filter',
        function ($scope, $state, $resource, $stateParams, NgTableParams, ApiQuery, $rootScope, $http, $filter) {
            ...
            ApiQuery.post("/GET_DETAIL_ORDER", {ID_ORDER: $stateParams.idOrder}).then(async function (res) {
                ...
                $scope.theOrder.DELIVER_DATE = res.data.RESPONSE.DELIVER_DATE.split('-')[2] + '/' + res.data.RESPONSE.DELIVER_DATE.split('-')[1] + '/' + res.data.RESPONSE.DELIVER_DATE.split('-')[0];
            ...
            $scope.submitPopup = async function (material, index) {
                ...
                $scope.theOrder.ITEMS.forEach(async function (value, index_item) {
                    if (value === $scope.theOrder.ITEMS[0]) {
                        if ($rootScope.item_global !== '000000000000000000') {
        ...
        function ($scope, $state, $resource, $stateParams, NgTableParams, ApiQuery, $rootScope, $http) {
            ...
            $scope.loadOrder = function (division, dc) {
                ApiQuery.post("/GET_TEMP", {id_user: $rootScope.userData.id_user}).then(function (res) {
                    $scope.templates = [];
                    if ($rootScope.userData.username === '0989728268' || $rootScope.userData.username === '0944588588') {
                        ...
                        res.data.RES.forEach(function (value) {
                            if ((value.DIVISION === division && $scope.sale_org === '1000' && value.ORDER_TYPE === "ZI01") || (value.DIVISION === division && $scope.sale_org === '3000' && (value.ORDER_TYPE === "ZI11" || value.ORDER_TYPE === 'ZI09'))
                                || $rootScope.userData.username === '0989728268' || $rootScope.userData.username === '0944588588') {
                                $scope.templates.push(value);
                            }
                        })
                    }
            ...
            $scope.createManual = function (templateId, dc, sale_org) {
                ...
                if (tem_index !== -1) {
                    ...
                    if ((sale_org === '1000' && $scope.templates[tem_index].ORDER_TYPE === "ZI01") || (sale_org === '3000' && ($scope.templates[tem_index].ORDER_TYPE === "ZI11" || $scope.templates[tem_index].ORDER_TYPE === "ZI09"))) {
                        ApiQuery.post('/UPDATE_DC_TEMP', {DC: dc, ID_TEMP: templateId}).then(function (res) {
    ...
    ['$scope', '$state', '$resource', '$stateParams', 'NgTableParams', 'ApiQuery', '$rootScope', '$http',
        function ($scope, $state, $resource, $stateParams, NgTableParams, ApiQuery, $rootScope, $http) {
            ...
            $scope.submit = function (status) {
                ...
                $scope._msg = {
                    date: "Chọn ngày yêu cầu giao hàng"
    ...
    ['$scope', '$state', '$resource', '$stateParams', 'NgTableParams', 'ApiQuery', '$rootScope', '$http',
        function ($scope, $state, $resource, $stateParams, NgTableParams, ApiQuery, $rootScope, $http) {
            ...
            ApiQuery.post('/GET_DETAIL_TEMP', {ID_TEMP: $stateParams.temID}).then(function (res) {
                ...
                ApiQuery.post('/CHECK_CHAR', {
                    ID_TEMPLATE_CLONE: res.data.RESPONSE.ID_TEMP_CLONE,
                }).then(function (res) {
                    $rootScope.check_char = res.data.GT_CHECK_CHAR;
                });
            ...
            $scope.checkDate = function (date1, date2) {
                let from1 = date1.split("/");
                let from2 = date2.split("/");
                let date1_convert = new Date(from1[2], from1[1] - 1, from1[0]);
                let date2_convert = new Date(from2[2], from2[1] - 1, from2[0]);
                if (date1_convert < date2_convert) {
                    $scope.message = 'Thêm phí can thiệp gấp';
                } else {
                    $scope.message = '';
                }
            };
            ...
            $scope.loadDientichThanCua = function () {
                if ($rootScope.characters.findIndex(item => item.ATNAM === 'Z_DIEN_TICH') !== -1) {
                    $rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_DIEN_TICH')].VALUE = parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_WIDTH')].VALUE) * parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_HEIGHT')].VALUE);
                    $rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_DIEN_TICH')].VALUE = parseFloat($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_DIEN_TICH')].VALUE / 1000000);
                    if ($rootScope.check_char[0] !== undefined) {
                        if ($rootScope.check_char[0].DIEN_TICH_MIN > 0 && $rootScope.check_char[0].DIEN_TICH_MAX > 0) {
                            let min = Math.min.apply(Math, $rootScope.check_char.map(function (o) {
                                return o.DIEN_TICH_MIN;
                            }));
                            let max = Math.max.apply(Math, $rootScope.check_char.map(function (o) {
                                return o.DIEN_TICH_MAX;
                            }));
                            if ($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_DIEN_TICH')].VALUE > max) {
                                $rootScope.message1 = 'Vượt quá diện tích';
                            } else {
                                if ($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_DIEN_TICH')].VALUE < min) {
                                    $rootScope.message1 = 'Thiếu diện tích';
                                } else {
                                    $rootScope.message1 = '';
                                }
                            }
                            ...
                            let cao_min = Math.min.apply(Math, $rootScope.check_char.map(function (o) {
                                return o.CAO_PB_MIN;
                            }));
                            let cao_max = Math.max.apply(Math, $rootScope.check_char.map(function (o) {
                                return o.CAO_PB_MAX;
                            }));
                            let rong_min = Math.min.apply(Math, $rootScope.check_char.map(function (o) {
                                return o.RONG_PB_MIN;
                            }));
                            let rong_max = Math.max.apply(Math, $rootScope.check_char.map(function (o) {
                                return o.RONG_PB_MAX;
                            }));
                            if (parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_WIDTH')].VALUE) > rong_max) {
                                $rootScope.message1 = 'Vượt quá chiều rộng';
                            } else {
                                if (parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_WIDTH')].VALUE) < rong_min) {
                                    $rootScope.message1 = 'Thiếu chiều rộng';
                                } else {
                                    $rootScope.message1 = '';
                                }
                            }

                            if (parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_HEIGHT')].VALUE) > cao_max) {
                                $rootScope.message2 = 'Vượt quá chiều cao';
                            } else {
                                if (parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_HEIGHT')].VALUE) < cao_min) {
                                    $rootScope.message2 = 'Thiếu chiều cao';
                                } else {
                                    $rootScope.message2 = '';
                                }
                            }
            ...
            $scope.addItemBT = function (value, characters) {
                ApiQuery.post('/GET_MARA', {
                    ID_USER: $rootScope.userData.id_user,
                    IT_MARAS: [{
                        MATNR: value
            ...
            $scope.addItemTR = function (value, characters) {
                ApiQuery.post('/GET_MARA', {
                    ...
                    let item_tr = {
                        ...
                        NAME: res_tr.data.MARAS[0].MAKTX,
                        ZTERM: '',
                        QUANTITY: parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_THANH_RAY')].VALUE) * parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_KT_RAY0')].VALUE) / 1000 ? parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_THANH_RAY')].VALUE) * parseInt($rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_KT_RAY0')].VALUE) / 1000 : 0,
                        SALE_UNIT: res_tr.data.MARAS[0].MEINS,
                        Z_RETURN: false,
                        ZPRICE: res_tr.data.MARAS[0].ZPRICE,
                        check: 'tr'
            ...
            $scope.submitPopup = async function (material, index) {
                if (($scope.theOrder.ITEMS[0].MATNR === material || material === undefined) && $scope.data.matnr_bt !== '') {
                    await $scope.addItemBT($scope.data.matnr_bt, $rootScope.characters);
                }
                if (($scope.theOrder.ITEMS[0].MATNR === material || material === undefined) && $scope.data.matnr_tr !== '') {
                    await $scope.addItemTR($scope.data.matnr_tr, $rootScope.characters);
                }
                if (($scope.theOrder.ITEMS[0].MATNR === material || material === undefined) && $scope.data.matnr_khoa !== '') {
                    await $scope.addItemKhoa($scope.data.matnr_khoa, $rootScope.characters);
                }
                $scope.theOrder.VKORG = $rootScope.sale_org;
                if ($scope.theOrder.ITEMS[0].CHARACTERISTICS.findIndex(item => item.ATNAM === 'Z_DIEN_TICH') !== -1
                ...
                if (index === 0 || index === undefined) {
                    $scope.theOrder.ITEMS[0].CHARACTERISTICS = $rootScope.characters;
                    $scope.theOrder.ITEMS[0].CHARACTERISTICS.forEach(function (value) {
                        if (value.VALUE === null) {
                            value.VALUE = "";
                        }
                    });

                    let index_model = $scope.theOrder.ITEMS[0].CHARACTERISTICS.findIndex(item => item.ATNAM === 'Z_MODEL_CUA');
                    let index_mau = $scope.theOrder.ITEMS[0].CHARACTERISTICS.findIndex(item => item.ATNAM === 'Z_MAU_SAC');
                    if ((index_model !== -1 || index_mau !== -1) && $scope.theOrder.ITEMS[0].CHARACTERISTICS[index_model].VALUE === '') {
                        bootbox.alert({
                            message: "Mời chọn loại cửa và màu sắc!!!",
                            callback: function () {
                            }
                        })
                    } else {
                        ApiQuery.post('/mapping_mara', {
                            ...
                            GT_CHARACTER: $scope.theOrder.ITEMS[0].CHARACTERISTICS
                        }).then(function (res) {
                            if (res.data.E_MESSAGE !== '') {
                                bootbox.confirm({
                                    title: "Đơn hàng ngoài tiêu chuẩn",
                                    ...
                                    callback: function (result) {
                                        if (result === true) {
                                            ...
                                            $('#myModal').modal('hide');
                                            if (res.data.MATNR_REFER !== '') {
                                                $scope.theOrder.ITEMS[0].MATNR = res.data.MATNR_REFER;
                                                $scope.dkgh($rootScope.marasX, res.data.MATNR_REFER);
                                                ...
                                                ApiQuery.post('/GET_MARA', {
                                                    ...
                                                    $scope.theOrder.ITEMS[0].EXTWG = res_mapping.data.MARAS[0].EXTWG;
                                                    $scope.theOrder.ITEMS[0].NAME = res_mapping.data.MARAS[0].MAKTX;
                                                    $scope.theOrder.ITEMS[0].ZPRICE = res_mapping.data.MARAS[0].ZPRICE;
                                                });
                                ...
                                let index_kttyc = $scope.theOrder.ITEMS[0].CHARACTERISTICS.findIndex(item => item.ATNAM === 'Z_KT_TRUC0');
                                if (res.data.E_FLAG === '1' || (index_kttyc !== -1 && ($scope.theOrder.ITEMS[0].CHARACTERISTICS[index_kttyc].VALUE !== '' && $scope.theOrder.ITEMS[0].CHARACTERISTICS[index_kttyc].VALUE !== null))) {
                                    $scope.theOrder.ztc = '1';
                                } else {
                                    // $scope.theOrder.ztc = '0';
                                }
                ...
                if ($rootScope.message1 !== '' || $rootScope.message2 !== '' || $rootScope.message3 !== '' || $rootScope.message4 !== '') {
                    $scope.theOrder.ztc = '1';
                }
            };
            ...
            $scope.addNew = function () {
                if ($scope.theOrder.ITEMS[0] !== undefined && $scope.theOrder.ITEMS[0].EXTWG !== '' && $scope.theOrder.ITEMS[0].EXTWG !== undefined) {
                    console.log("here");
                    ApiQuery.post('/GET_MARA', {
                        ID_USER: $rootScope.userData.id_user,
                        ZTYPE: $scope.theOrder.ITEMS[0].EXTWG,
            ...
            $scope.addNewNH = function (ztype_nh) {
                if (ztype_nh === undefined || ztype_nh === '') {
                    bootbox.alert({
                        message: "Mời chọn loại mặt hàng",
                    })
                } else {
                    ApiQuery.post('/GET_MARA', {
                        ID_USER: $rootScope.userData.id_user,
            ...
            $scope.loadBoToi_ThanhRay = function (group, value2, atnam) {
                ...
                if (group === '01' && (atnam === 'Z_MODEL_CUA')) {
                    $scope.default_maras.forEach(function (value) {
                        if ($rootScope.type === "edit") {
                            if (value.ZMBEZ === 'Z_MAU_SAC') {
                                $scope.maras_unremove.push(value);
                            }
                        } else {
                            if (value.ZMBEZ === 'Z_MODEL_CUA' && value.VALUE === value2) {
                                let index = $scope.default_maras.findIndex(item => item.ID_MATNR_REFER === value.ID_MATNR_REFER && item.ZMBEZ === 'Z_MAU_SAC');
                    ...
                    $scope.maras_unremove = $scope.maras_unremove.reduce((unique, o) => {
                ...
                if (group === '01' && atnam === 'Z_MAU_SAC') {
                    let model_cua = $rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_MODEL_CUA')].VALUE;
                    let index = 0;
                    if ($rootScope.check_char[0] !== undefined) {
                        if ($rootScope.check_char[0].DIEN_TICH_MIN > 0 && $rootScope.check_char[0].DIEN_TICH_MAX > 0) {
                            let dien_tich = $rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_DIEN_TICH')].VALUE;
                            index = $rootScope.check_char.findIndex(item => item.MODEL_CUA === model_cua && item.DIEN_TICH_MIN < dien_tich && item.DIEN_TICH_MAX > dien_tich);

                        } else {
                            let cao = $rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_HEIGHT')].VALUE;
                            let rong = $rootScope.characters[$rootScope.characters.findIndex(item => item.ATNAM === 'Z_WIDTH')].VALUE;
                            index = $rootScope.check_char.findIndex(
                                item => item.MODEL_CUA === model_cua
                                    && item.RONG_PB_MIN < rong && item.RONG_PB_MAX > rong
                                    && item.CAO_PB_MIN < cao && item.CAO_PB_MAX > cao);
                        }

                        if (index > -1) {
                            let check = $rootScope.check_char[index];
                            let position = check.MAU_SON.search(value2);
                            if (position < 0) {
                                $rootScope.message3 = 'Chọn sai màu sắc';
                            } else {
                                $rootScope.message3 = '';
                            }
                        }
                ...
                if (group === '07' && atnam === 'Z_MODEL_BT') {
                    ...
                    //     if (index > -1) {
                    //         let check = $rootScope.check_char[index];
                    //         let position = check.LOAI_MOTOR.search(value2);
                    //         if (position < 0) {
                    //             $rootScope.message4 = 'Chọn sai loại bộ tời';
                    //         } else {
                    //             $rootScope.message4 = '';
                    //         }
                    //     }
            ...
            $scope.refresh_data = function (atnam, flag, val) {
                switch (flag) {
                    case "botoi":
                        $scope.data.matnr_bt = "";
                        break;
                ...
                ;
                if (atnam === 'Z_KHOA') {
                    $scope.data.matnr_khoa = "";
                }
                if (flag === '' || flag === undefined || atnam === 'Z_KHOA') {
                    let index = $scope.theOrder.ITEMS[0].CHARACTERISTICS.findIndex(item => item.ATNAM === atnam);
                    $scope.theOrder.ITEMS[0].CHARACTERISTICS[index].VALUE = '';
                }
                ...
                if (jQuery(".js-validation-material").valid()) {
                    if ($scope.vsart === undefined || $scope.vsart === '' || $scope.route === undefined || $scope.route === '') {
                        ...
                        if ($scope.theOrder.DIVISION === '02' && $rootScope.excel.value !== 'NHCCBH' && $rootScope.excel.value !== 'PKNH') {
                            $scope.theOrder.ITEMS.forEach(function (value) {
                                if ((value.CHARACTERISTICS.find(o => o.ATNAM === 'Z_BOS').VALUE === '0' || value.CHARACTERISTICS.find(o => o.ATNAM === 'Z_BOS').VALUE === '' || value.CHARACTERISTICS.find(o => o.ATNAM === 'Z_BOS').VALUE === undefined) && (value.thanh_le === '0' || value.thanh_le === undefined)) {
                                    $scope.theOrder.ITEMS = $scope.theOrder.ITEMS.filter(item => item !== value);
                                }
                                ;
                            })
